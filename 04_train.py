...
# train_discriminator
def train_discriminator(generator_model, discriminator_model, ja_seq_len, x_train, y_train, batch_size=128):
    ...
    # TODO: 本物の英語文/日本語文データをja_seq_len数に増幅
    ...
    # discriminatorの学習
    discriminator_model.train_on_batch([en_input, ja_input], y)
...
# train
def train(generator_model, discriminator_model, gan_model, ja_seq_len, x_train, y_train, step=1000, batch_size=128):

    # TODO: 学習時の損失関数の値を格納する箱を用意

    # loop (=epoch)
    for _ in tqdm(range(step)):
        train_discriminator(generator_model, discriminator_model, ja_seq_len, x_train, y_train, batch_size)
